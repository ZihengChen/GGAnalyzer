...
f = r.TFile(channel+"_prefit.root","recreate")
################################################
...
if channel in ['mutau']:
    fileList = fileList + [('data', '%s/SingleMu.root' %dirName)]
if channel in ['etau']:
    fileList = fileList + [('data', '%s/SingleEl.root' %dirName)]


variableName = ""
bining = []
xs_calculator(fileList, 40, 120, 30, channel+"_visibleMass", [30, 0, 300])
